...
if __name__ == '__main__':
    ...
    print(f"Final members: T={sim.global_state['n_active_traffickers']}, P={sim.global_state['n_active_packagers']}, R={sim.global_state['n_active_retailers']}")
    print(f"Total revenues: €{sim.data_collector.data['revenues'][-1] if sim.data_collector.data['revenues'] else 0:.2f}")
    print(f"Total expenses: €{sim.data_collector.data['expenses'][-1] if sim.data_collector.data['expenses'] else 0:.2f}")
