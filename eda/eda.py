...
def _(Path, mo):
    ...
    dir_to_name_map = {f"{full_path.parent.name}/{full_path.name}" : full_path  for full_path in results_path.rglob("*.parquet")}
...
def _(mo, pl, px):
    ...
    def make_line(df: pl.DataFrame, y_cols: list[str], title: str, y_title: str):
        ...
        plot_df = df_sorted.select([tick_col] + y_cols).unpivot(
            index=tick_col, variable_name="metric", value_name="value"
        )
...
def _(chosen_scenario, make_line, mo):
    cash_box_plot = make_line(
        chosen_scenario, ["cash_box"], "Cash box over tick", "cash_box"
     )
    n_components_plot = make_line(
        chosen_scenario, ["n_components"], "Number of components over tick", "n_components"
     )
