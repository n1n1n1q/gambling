...
class ExperimentRunner:
    ...
    def run_arrest_scenarios(self, arrest_scenarios: List[int] = None, 
                            num_simulations: int = 10,
                            num_workers: int = None,
                            disruption_mode: str = "scenario1",
                            efficiency_vs_security: float = 0.5) -> Dict:
        """
        ...
        for scenario in arrest_scenarios:
            ...
            tasks = [(scenario, disruption_mode, efficiency_vs_security) 
            ...
            # Aggregate results
            results[scenario] = self._aggregate_scenario_results(scenario_results, scenario)
            
            active_count = sum(1 for r in scenario_results if r['final_running'])
            logger.info(f"  Active organizations at end: {active_count}/{num_simulations}")
    ...
    def _aggregate_scenario_results(self, scenario_results: List[Dict], scenario: int) -> Dict:
        """Aggregate results across simulations in a scenario"""
        
        active_count = sum(1 for r in scenario_results if r['final_running'])
        
        # Aggregate financial metrics
        cash_boxes = [r['final_cash_box'] for r in scenario_results if r['final_running']]
        ...
        aggregated = {
            ...
            'avg_final_cash_box': sum(cash_boxes) / len(cash_boxes) if cash_boxes else 0,
            'median_final_cash_box': sorted(cash_boxes)[len(cash_boxes)//2] if cash_boxes else 0,
    ...
    def print_summary(self):
        ...
        for scenario in sorted(self.results.keys()):
            ...
            print(f"  Active Organizations (end): {data['num_active_at_end']} ({data['survival_rate']*100:.1f}%)")
            print(f"  Avg Final Cash Box: €{data['avg_final_cash_box']:,.2f}")
            print(f"  Median Final Cash Box: €{data['median_final_cash_box']:,.2f}")


def run_single_sim(arrest_scenario: int, disruption_mode: str, 
                   efficiency_vs_security: float) -> Dict:
    """
...
class SimulationComparison:
    ...
    def compare_efficiency_levels(self, arrest_scenarios: List[int] = None,
                                  num_simulations: int = 5) -> Dict:
        """
